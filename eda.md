...
Could group\_by within a location value and set of month values
...
**NEVER** use base R’s `table`. Result is not a dataframe. Never pull
variables out of dataframe.
...
Suppose you want to summarize many columns. If apply same summarize
function to multiple variables using `across` within `summarize()`
...
Centering around a group mutate. If forgot you grouped, can get into
trouble.
...
Take new observations, copy to new variable and then down by 1. Helpful
if want to compute lag from 1 day to next. OR see if yesterday’s temp
predicts todays.
...
```
    ...
    ## # A tibble: 3 x 4
    ##   name           temp_change_max temp_change_mean temp_change_sd
    ##   <chr>                    <dbl>            <dbl>          <dbl>
    ## 1 CentralPark_NY            12.7         -0.0409            4.45
    ## 2 Waikiki_HA                 6.7         -0.00140           1.23
    ## 3 Waterhole_WA               8            0.0206            3.13
